...
Use this workflow when executing against an approved workflow objective.

## Workflow

1. Check the objective's `status:` — must be ≥ `approved` (ordinal 2). If not, refuse and tell the user to finish planning first (suggest **review** gate-review).
...
- Follow **docs** conventions for Timeline entries and document updates.
