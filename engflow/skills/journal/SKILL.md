...
Events write to `{journal-dir}/{project}/events.log`. When the journal lives inside `.eng/` and EngDirs is configured, events sync automatically via push. One push = everything.
...
- **push** — syncs `.eng/` (including journal when colocated) to EngDirs remote
- **docs** — engineering documentation system (objectives, findings, retros)
